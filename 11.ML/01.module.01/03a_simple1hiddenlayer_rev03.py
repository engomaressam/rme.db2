...
def train(Y, X, model, optimizer, criterion, epochs=800, doc=None, fig_dir="figs_rev03"):
    cost = []
    total = 0
...
# Make some data (DIFFERENT from course)
X = torch.arange(-30, 30, 2).view(-1, 1).type(torch.FloatTensor).to(device)
...
doc = Document()
doc.add_heading('Simple One Hidden Layer Neural Network - GPU Version (Different Params)', 0)
doc.add_paragraph(f"Device used: {device}")

# Train the model (DIFFERENT params)
D_in = 1
...
doc.add_heading('Training with Cross Entropy Loss (Different Params)', level=1)
cost_cross = train(Y, X, model, optimizer, criterion_cross, epochs=800, doc=doc)
...
loss_fig_path = "figs_rev03/loss_curve.png"
plt.savefig(loss_fig_path)
plt.close()
doc.add_paragraph("Loss curve:")
doc.add_picture(loss_fig_path, width=Inches(4))

# Predictions (DIFFERENT values)
x = torch.tensor([5.0]).to(device)
yhat = model(x)
doc.add_paragraph(f'Prediction for x=5.0: {yhat.detach().cpu().numpy()}')

X_ = torch.tensor([[5.0], [15.0], [-15.0]]).to(device)
Yhat = model(X_)
...
doc.save("03a_simple1hiddenlayer_results_rev03.docx")
print("Results saved to 03a_simple1hiddenlayer_results_rev03.docx") 