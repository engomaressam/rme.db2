...
class SimilarVideosDetector:
    ...
    def calculate_video_fingerprint(self, video_info):
        ...
        try:
            ...
            # Method 1: Try with OpenCV first
            try:
                cap = cv2.VideoCapture(video_info.path)
                if not cap.isOpened():
                    error_messages.append("OpenCV could not open video")
                else:
                    # Get video properties
                    total_frames = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
                    if total_frames <= 0:
                        error_messages.append(f"Invalid frame count: {total_frames}")
                        cap.release()
                    else:
                        success = True
            except Exception as e:
            ...
            # If OpenCV failed, try with MoviePy as backup
            if not success:
                try:
                    clip = VideoFileClip(video_info.path, audio=False)
                    
                    # Get video properties from MoviePy
                    ...
                    error_messages.append(f"MoviePy error: {str(e)}")
            ...
            # If we got here with success=True, use the OpenCV method
            if success:
            ...
            raise Exception(f"Failed to extract frames: {'; '.join(error_messages)}")
            
        except Exception as e:
            logging.error(f"Error calculating fingerprint for {video_info.path}: {str(e)}")
            return None
...
def select_directory():
    ...
    try:
        ...
        if directory:  # Only if directory was selected
            # Make root visible again for our dialog
            try:
                root.deiconify()
            except:
                # If there's an issue with the window, create a new one
                root.destroy()
                root = tk.Tk()
        ...
        logging.error(f"Error in directory selection: {str(e)}")
...
def handle_problematic_video(video_path, error_message):
    """Handle a problematic video file by logging the error and optionally notifying the user."""
    # Log the error
    logging.error(f"Problem with video file: {video_path}")
    ...
    return f"Skipped problematic file: {os.path.basename(video_path)}\nError: {error_message}"
...
def main():
    ...
    parser.add_argument("--skip-errors", action="store_true", help="Skip problematic files and continue processing")
    ...
    root.destroy()  # Now we can destroy the root window
    ...
    try:
        ...
        try:
            root_exists = root is not None and root.winfo_exists()
        except Exception:
            root_exists = False
        ...
        logging.error(f"Error in GUI initialization: {str(e)}")
        print(f"\nError initializing GUI: {str(e)}\nResults are available in the report file.")
        # Still try to show the report path
        try:
            messagebox.showinfo("Report Saved", f"GUI initialization failed, but results are available in:\n{detector.report_path}")
        except:
            pass
...
if __name__ == "__main__":
    try:
        ...
        # If running in GUI mode, show error message
        try:
