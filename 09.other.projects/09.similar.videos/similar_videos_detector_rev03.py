...
class SimilarVideosDetector:
    ...
    def save_report(self, output_file=None):
        """Save a report of similar videos to a file."""
        if output_file is None:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            output_file = f"similar_videos_report_{timestamp}.txt"
...
class VideoComparisonGUI:
    ...
    def display_current_cluster(self):
        ...
        # Update status label
        self.status_label.config(text=f"Displaying {len(cluster)} similar videos")
        ...
        for i, video in enumerate(cluster):
            ...
            video_frame.grid(row=0, column=i, padx=10, pady=10, sticky="n")
            
            # Frame for thumbnails
            thumbnails_frame = ttk.Frame(video_frame)
            thumbnails_frame.pack(padx=5, pady=5)
            
            # Display all 3 thumbnails side by side
